...
public class TouchingObjects : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            Destroy(collision.gameObject);
            if (FindObjectOfType<ResultController>().currentLevelTime < 5)
            {
                FindObjectOfType<ResultController>().currentLevelTime = 0;
            }
            else
            {
                FindObjectOfType<ResultController>().currentLevelTime -= 5;
            }
        }
